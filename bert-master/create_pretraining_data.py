...
log.setLevel(logging.INFO)  # 控制台显示控制, INFO ,WARN
...
flags.DEFINE_integer("max_predictions_per_seq", 4,
  ...
  for input_file in input_files:
    with tf.gfile.GFile(input_file, "r") as reader:
      while True:
        ...
        if not line:  # 空行，表示新的文档
          all_documents.append([])
        tokens = tokenizer.tokenize(line)
        if tokens:
          all_documents[-1].append(tokens)
          tf.logging.info(f'当前 all_documents:{all_documents}')

  # Remove empty documents
  all_documents = [x for x in all_documents if x]
  rng.shuffle(all_documents)
  tf.logging.info(f'documents数量:{len(all_documents)}')
  tf.logging.info(f'语料数量:{len([j for i in all_documents for j in i])}')
  tf.logging.info(f'打乱 all_documents:{all_documents}')
...
def create_instances_from_document(
  ...
  tf.logging.info(f'target_seq_length：{target_seq_length}')
  ...
  while i < len(document):
    ...
    if i == len(document) - 1 or current_length >= target_seq_length:
      if current_chunk:
        ...
        tf.logging.info(f'原始tokens：{tokens}')
...
def main(_):
  tf.logging.info(f'=============传参：')
  tf.logging.info(FLAGS.input_file)
  tf.logging.info(FLAGS.output_file)
  tf.logging.info(FLAGS.vocab_file)
  tf.logging.info(f'do_lower_case:  {FLAGS.do_lower_case}; TRUE--=转小写')
  tf.logging.info(f'max_seq_length: {FLAGS.max_seq_length}')
  tf.logging.info(f'dupe_factor:  {FLAGS.dupe_factor}')
  tf.logging.info(f'do_whole_word_mask: {FLAGS.do_whole_word_mask}')
  tf.logging.info(f'=============done：')
