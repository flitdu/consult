...
```python
dftrain_raw['Survived'].value_counts()  # 生存人数的数量统计
```
...
```python
# pandas query 用法, 操作列
f = pd.DataFrame({'A': range(1, 6),
                  'B': range(10, 0, -2),
                  'C': range(10, 5, -1)})
print(f)
f.query('A>B')  # 查询A列>B 列结果
```
...
* Embarked:乘客登船港口:S、C、Q(有缺失)【转换成onehot编码，四维度 S,C,Q,nan】
    ...
    #Pclass,票类，转one-hot
    dfPclass = pd.get_dummies(dfdata['Pclass'])  # 转one_hot
    dfPclass.columns = ['Pclass_' +str(x) for x in dfPclass.columns ]  # 重命名列
    ...
    #Embarked, 港口情况，注意存在缺失，这里处理方法添加了’dummy_na=True‘
...
```python
x_train.loc[x_train.isnull().T.any(), :]  # 显示存在nan 的行
```
...
```python
tf.keras.backend.clear_session()  # 销毁当前的TF图并创建一个新图
